...
function AuthordCard({ className, data }: Props) {
	const imgSrc = `${data.thumbnail.url}&fm=webp`;
	const placeholder = `${data.title} is the author of this article. Feel free to leave comments and feedbacks below`;

	return (
		<article
			className={className}
			tw="flex flex-col items-center bg-gray-100 px-5 pt-12 pb-10 md:(pt-12 px-11 pb-11) xl:(flex-row items-start)"
		>
			<div tw="xl:(-ml-24 mr-8)">
				<Image
					...
					tw="w-28 h-28 rounded-full object-cover"
				/>
			</div>
			...
			<div tw="space-y-5 flex-1 mt-2 text-center xl:text-left flex flex-col items-center xl:(items-start mt-0)">
				<header>
					<p tw="text-gray-200 font-light text-smaller">Written By</p>
					{/* TODO link to author page */}
					<a tw="text-xl md:text-3xl font-black">{data.title}</a>
				</header>
